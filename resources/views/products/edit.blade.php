						...
						<div class="form-group">
							...
							<div class="col-sm-9"> 
								<select name="partner_id" class="form-control">
									...
									<option @if($product->partner_id== $partner->id){{"selected"}} @endif value="{{ $partner->id }}">{{ $partner->partner_name }}</option>
									@endforeach
								</select>
							</div>
						</div>
					...
					<div class="col-sm-7">
						<div class="row">
							<div class="col-sm-8 text-center" style="margin-top: 50px;">
								<div class="form-group">
									{!! Form::button('<i class="fa fa-check-circle-o" aria-hidden="true"></i> Save changes',['type'=>'submit', 'class'=>'btn btn-primary']) !!}
									<a href="{{ route('products.index') }}" class="btn btn-danger"><i class="fa fa-repeat" aria-hidden="true"></i> Cancel</a>
								</div>
							</div>
							<div class="col-sm-4">
								<img src="{{asset('uploads/'.$product->image)}}" class="pull-right" id="showimage" width="200" height="200" style="border: 1px solid #ddd;" onerror="this.src="{{asset('images/default.jpg')}}">
							</div>
						</div>							
					</div>
				</div>	
